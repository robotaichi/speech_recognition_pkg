...
from speech_recognition_pkg.srv import play_end_check_service, speech_recognition_service, openjtalk_service # サービスファイルの読み込み（from パッケージ名.srv import 拡張子なしサービスファイル名）
import speech_recognition as sr #音声認識に必要
...
class Speech_recognition_Server(): #サーバーのクラス
    def __init__(self):
    ...
    def make_msg(self): #送信するメッセージの作成
            ...
            recognized_text = unicode("たぶん2つだと思う", "utf-8") #デバッグ用
            self.service_message.speech_recognition_response = u"{}".format(recognized_text).encode("utf-8") #voice_record関数により音声認識した結果を日本語も扱えるutf-8型にエンコード



    def success_log(self, req): #成功メッセージの表示（callback関数）
        self.make_msg()
        return self.service_message.speech_recognition_response #srvファイルで定義した返り値をsrvに渡す。rospy.Serviceによって呼び出された関数（callback関数）内でreturnすること



    def service_response(self): #サービスの応答
        srv = rospy.Service('speech_recognition_service', speech_recognition_service, self.success_log) #サービスのリクエストがあった場合にsuccess_log関数（callback関数）を呼び出し、実行。呼び出し先の関数内で返り値をreturnする必要がある
...
def main(): #メイン関数
    ...
    vrs = Speech_recognition_Server() #クラスのインスタンス作成（クラス内の関数や変数を使えるようにする）
    vrs.service_response() #サービスの応答
    rospy.spin() #終了防止
